...
contract StrategyTargetPrices is Swapper {
    ...
    function sellStrategyAsset(uint256 total, uint256 bpsToSell) internal returns (uint256 price, uint256 receivedAmount) {
        ...
        if(strategyAsset == Constants.WETH9) {
            return swapExactInputMultihop(strategyAsset, amountToSwap, Constants.SELL_WETH_PATH);
        } else {
            return swapExactInputMultihop(strategyAsset, amountToSwap, Constants.SELL_RETH_PATH);
        }
    }
